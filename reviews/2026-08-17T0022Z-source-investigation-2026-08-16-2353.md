...
- `user.name` / `user.email` set to the tool vendor's bot identity (a vendor
  no-reply address);
...
That is a tool committer identity plus a vendor-keyed signature, both applied
automatically to every commit. A session-start hook installed by the execution
environment (a session-start identity script) is what sets them, and
...
`Co-authored-by` trailer, no `Generated with` line, and no tool-vendor identity of
any kind; `devtools/repo-scan`'s "tool attribution" check passes on the tree
