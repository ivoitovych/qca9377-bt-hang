...
| CB-02 | GitLint FAIL: B3 hard tabs on every quoted C line of both messages (16 / 11 lines); BlueZ's `.gitlint` does not relax B3 | **open — operator's decision**: a v2 with the quoted code indented by spaces clears it; sending a v2 for a linter before a maintainer has replied may be noise. **v2 prepared** in `patches/bluez/v2/` (gitlint 0, checkpatch 0/0, `git am` 6/6), not sent | `patches/bluez/README.md` §Sent | `awk '/^---$/{exit} /\t/' patches/bluez/000?-*.patch \| wc -l` → 0 once re-indented |
