...
install_file() {
    ...
    # ⚠️ A `.disabled` SIBLING MEANS bt-mode MOVED THIS FILE ASIDE ON PURPOSE.
    # `bt-mode experiment` implements the controlled baseline by renaming the
    # modprobe override and the udev power pin to `.disabled`. Reinstalling the
    # active name beside it silently reverts that baseline — which --tools-only
    # did on 2026-08-19 and again on 09-01, while its refusal text promised it
    # "arms nothing, so the mode stands" (R2-58). Every trial from then on ran
    # under autosusp=N/power=on with a mode stamp still reading `experiment`.
    # The file is left exactly as bt-mode left it; the deploy says so and goes
    # on. Overridable only by leaving experiment mode, never by a flag here.
    if [[ -e "$DESTDIR$dst.disabled" ]]; then
        echo "  · $dst SKIPPED — a .disabled sibling exists (bt-mode moved it aside);"
        echo "    leave experiment mode with 'bt-mode mitigation' to reinstall it"
...
if [[ -r "$MODE_STAMP" ]] && grep -q '^experiment' "$MODE_STAMP" 2>/dev/null && (( ! TOOLS_ONLY )); then
    ...
    if [[ "${BT_FORCE_INSTALL:-0}" != 1 ]]; then
        ...
        echo "   That deploys the tools and leaves any file bt-mode moved aside"
        echo "   (a .disabled sibling) exactly where it is, so the mode stands."
        echo "   (Until 2026-09-16 it reinstalled those two files — R2-58.)"
        echo
...
# Same rule as install_file(), restated here because this file is generated by
# redirect and never passes through it: a `.disabled` sibling is bt-mode's
# doing, and writing the active name beside it is the 08-19 / 09-01 baseline
# reversion (R2-58). Checked before APPLY so the dry run reports it too.
if [[ -e "$UDEV.disabled" ]]; then
    echo "  · ${UDEV#"$DESTDIR"} SKIPPED — a .disabled sibling exists (bt-mode moved it aside)"
    SKIPPED_DISABLED=$((SKIPPED_DISABLED + 1))
elif (( APPLY )); then
...
# Reported for EVERY mode, not only tools-only: a skipped file is a fact about
# the machine's baseline, and the one time nobody said it out loud cost four
# weeks of trials their comparability (R2-58).
if (( SKIPPED_DISABLED > 0 )); then
    echo "  ⚠️  $SKIPPED_DISABLED file(s) left as bt-mode moved them aside (.disabled sibling present)."
    echo "      The experiment baseline is UNCHANGED by this deploy. 'bt-mode status' shows it."
    echo
fi
