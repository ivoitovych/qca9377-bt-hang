...
| EX-004 | [usb-reset-alone-is-insufficient](004-usb-reset-alone-is-insufficient.md) | A USB-level reset applied BEFORE any HCI command timeout does re-enumerate the controller and bring the HCI stack back up, but does not clear the controller's fault: the same desync recurs 107 ms later and the controller hangs completely 132 s afterwards. |
