...
| EX-006 | [sco-setup-is-the-wedging-command](006-sco-setup-is-the-wedging-command.md) | The controller stops answering HCI while servicing HCI_Setup_Synchronous_Connection (opcode 0x0428) — the SCO/eSCO link setup for HFP. The command is submitted, is never answered, and times out 2.169 s later. |
| EX-007 | [autonomous-profile-switch-triggers-hang](007-autonomous-profile-switch-triggers-hang.md) | The hang requires NO operator action. It is triggered by audio playback STOPPING: when the A2DP stream goes idle, PipeWire switches the device to the HFP profile 36 ms later, BlueZ issues HCI_Setup_Synchronous_Connection, and the controller never answers it. |
