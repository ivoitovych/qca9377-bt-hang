...
## ⚠️ A confound found 2026-08-23, and it weakens the central comparison

**The reboot crossed a kernel upgrade, and this exhibit did not record it.**

```console
$ journalctl -k -b -2 --no-pager -o cat | grep -m1 -oE 'Linux version [0-9.]+-[0-9]+-generic'
Linux version 7.0.0-29-generic
$ journalctl -k -b -1 --no-pager -o cat | grep -m1 -oE 'Linux version [0-9.]+-[0-9]+-generic'
Linux version 7.0.0-30-generic
```

The ladder ran under `7.0.0-29`; the boot that failed to enumerate was the **first boot of
`7.0.0-30`**. And `EX-027`, the exhibit this one contradicts, ran entirely under
`7.0.0-28`.

So the stage-1 comparison spans **three kernel versions**, not one, and the recovery ladder
is no longer the only recorded difference between the two stage-1 reboots. A kernel upgrade
is not an obvious way to break USB enumeration — but "not obvious" is precisely the standard
this record refuses elsewhere, and `n = 1` on each side leaves no way to separate them.

**What that costs.** The claim "the ladder left the controller unable to enumerate" drops
from *supported by the only recorded difference* to **one of two candidate explanations**.
The withdrawal of `EX-027`'s stage rule stands regardless — a stage-1 reboot did fail, and
that is what the rule forbade — but the *cause* is now open.

**Why it was missed.** This exhibit's provenance said `7.0.0-28-generic`, which was wrong
for both boots. It was hand-written rather than generated by `tools/bt-exhibit`, which reads
`uname -r` correctly; the value was copied forward from `EX-029`. A provenance field exists
to be checkable, and an unchecked one is worse than an absent one because it invites
reliance.
...
clean. ⚠️ Both statements survive the confound above; the *attribution to the ladder* does
not.
...
| kernel | **`7.0.0-29-generic` before the reboot, `7.0.0-30-generic` after** — corrected 2026-08-23; see the confound section |
